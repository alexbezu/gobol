...
type scanner_pli struct {
	...
	tok       tokens_pli
	lit       string       // valid if tok is _Name, _Literal, or SEMICOL ("semicolon", "newline", or "EOF"); may be malformed if bad is true
	bad       bool         // valid if tok is _Literal, true if a syntax error occurred, lit may be malformed
	kind      LitKind      // valid if tok is _Literal
	op        Operator_pli // valid if tok is _Operator, _AssignOp, or _IncOp
	//prec      int          // valid if tok is _Operator, _AssignOp, or _IncOp
}

func (s *scanner_pli) init(src io.Reader, errh func(line, col uint, msg string), mode uint) {
...
func (s *scanner_pli) errorf(format string, args ...interface{}) {
	s.error(fmt.Sprintf(format, args...))
}

// errorAtf reports an error at a byte column offset relative to the current token start.
func (s *scanner_pli) errorAtf(offset int, format string, args ...interface{}) {
	s.errh(s.line, s.col+uint(offset), fmt.Sprintf(format, args...))
}

// setLit sets the scanner state for a recognized _Literal token.
func (s *scanner_pli) setLit(kind LitKind, ok bool) {
	s.nlsemi = true
	s.tok = _Literal
...
func (s *scanner_pli) ident() {
	...
	// possibly a keyword
	lit := strings.ToUpper(string(s.segment()))
	if len(lit) >= 2 {
		if tok := keywordMap[lit]; tok != 0 && tokStrFast(tok) == string(lit) {
			// s.nlsemi = contains(1<<END|1<<RETURN, tok)
			// s.lit = fmt.Sprint(tok)
			s.tok = tok
			return
		}
	}
...
func tokStrFast(tok tokens_pli) string {
	return _tokens_pli_name[_tokens_pli_index[tok-1]:_tokens_pli_index[tok]]
}

func (s *scanner_pli) atIdentChar(first bool) bool {
	switch {
...
var keywordMap map[string]tokens_pli // size must be power of two

func init() {
	keywordMap = map[string]tokens_pli{}
	for tok := ZERODIVIDE; tok <= A; tok++ {
		h := tok.String()
		if keywordMap[h] != 0 {
...
func (s *scanner_pli) digits(base int, invalid *int) (digsep int) {
	if base <= 10 {
		max := rune('0' + base)
...
func (s *scanner_pli) number(seenPoint bool) {
	ok := true
	kind := IntLit
	base := 10        // number base
...
func (s *scanner_pli) comment(text string) {
	s.errorAtf(0, "%s", text)
}

func (s *scanner_pli) skipLine() {
...
func (s *scanner_pli) lineComment() {
...
func (s *scanner_pli) skipComment() bool {
	for s.ch >= 0 {
		for s.ch == '*' {
			s.nextch()
			if s.ch == '/' {
...
func (s *scanner_pli) fullComment() {
	/* opening has already been consumed */
...
func (s *scanner_pli) escape(quote rune) bool {
	var n int
	var base, max uint32
...
func (s *scanner_pli) preprocessor() {
	var ret = ""

	//cut off MF's markup
	for _, line := range strings.Split(string(s.buf), "\n") {
		ret += line[1:72] + "\n"
	}
	//remove PROCESS
	process_rgx := regexp.MustCompile("^PROCESS.+;")
	ret = process_rgx.ReplaceAllString(ret, "")
