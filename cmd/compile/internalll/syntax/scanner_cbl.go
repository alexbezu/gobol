...
func (s *scanner_cbl) init(src io.Reader, errh func(line, col uint, msg string), mode uint) {
...
func (s *scanner_cbl) errorf(format string, args ...interface{}) {
	s.error(fmt.Sprintf(format, args...))
}

// errorAtf reports an error at a byte column offset relative to the current token start.
func (s *scanner_cbl) errorAtf(offset int, format string, args ...interface{}) {
	s.errh(s.line, s.col+uint(offset), fmt.Sprintf(format, args...))
}

// setLit sets the scanner state for a recognized _Literal token.
func (s *scanner_cbl) setLit(kind LitKind, ok bool) {
	s.nlsemi = true
	s.tok = _Literal
...
// func tokStrFast(tok tokens_cbl) string {
// 	return _tokens_cbl_name[_tokens_pli_index[tok-1]:_tokens_pli_index[tok]]
// }

func (s *scanner_cbl) atIdentChar(first bool) bool {
	switch {
...
// var keywordMap [1 << 9]tokens_pli // size must be power of two
...
func (s *scanner_cbl) digits(base int, invalid *int) (digsep int) {
	if base <= 10 {
		max := rune('0' + base)
...
func (s *scanner_cbl) number(seenPoint bool) {
	ok := true
	kind := IntLit
	base := 10        // number base
...
func (s *scanner_cbl) comment(text string) {
	s.errorAtf(0, "%s", text)
}

func (s *scanner_cbl) skipLine() {
...
func (s *scanner_cbl) lineComment() {
...
func (s *scanner_cbl) skipComment() bool {
	for s.ch >= 0 {
		for s.ch == '*' {
			s.nextch()
			if s.ch == '/' {
...
func (s *scanner_cbl) fullComment() {
	/* opening has already been consumed */
...
func (s *scanner_cbl) escape(quote rune) bool {
	var n int
	var base, max uint32
